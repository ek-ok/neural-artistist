...
def get_image(image_path, content_image, style_image, vgg_params_path, iterations=10, alpha_beta_ratio=0.001):
    ...
    content_image1 = (content_image1 - np.min(content_image1)) / (np.max(content_image1) - np.min(content_image1))
    style_image1 = (style_image1 - np.min(style_image1)) / (np.max(style_image1) - np.min(style_image1))

    # normalize image to range of [0, 1]
    content_image1 = content_image1.reshape((1,) + content_image1.shape)
    style_image1 = style_image1.reshape((1,) + style_image1.shape)
